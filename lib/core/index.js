...
module.exports = function createMiddleware(_dir, _options) {
  ...
  return function middleware(req, res, next) {
    ...
    if (req.method === 'PUT' && parsed.pathname === '/__wisp/upload') {
      const pathQuery = parsed.query ? parsed.query.split('&').find(p => p.startsWith('path=')) : null;
      const nameQuery = parsed.query ? parsed.query.split('&').find(p => p.startsWith('name=')) : null;

      if (!pathQuery || !nameQuery) {
        res.statusCode = 400;
        res.end('Missing parameters');
        return;
      }

      const targetPath = decodeURIComponent(pathQuery.split('=')[1]);
      const filename = decodeURIComponent(nameQuery.split('=')[1]);

      const rootDir = path.resolve(opts.root);
      const destDir = path.normalize(path.join(rootDir, path.relative('/', targetPath)));
      const destFile = path.normalize(path.join(destDir, filename));
    ...
    if (parsed.pathname === '/__wisp/thumbnail') {
      const getThumbnail = require('./show-dir/thumbnail');
      getThumbnail(req, res, opts, parsed);
      return;
    }

    if (parsed.pathname.startsWith('/__wisp/')) {
      const relativePath = parsed.pathname.replace(/^\/__wisp\//, '');
      const safePath = path.normalize(relativePath).replace(/^(\.\.[\/\\])+/, '');
      const assetPath = path.join(__dirname, 'show-dir', 'assets', safePath);
